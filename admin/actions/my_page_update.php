...
if(!empty($_POST)) {
     ...
     * осуществляем проверку на совпадение с существующими логинами
     * если НЕсуществует  такого логина - 
     * совершаем запрос на изменение
     */
    if (!empty($login)) {
    ...
    if(empty($error)) {
        ...
        $sql->bindParam(':id', $_GET['id']);
        $sql->execute();
        /* Изменяем пароль только в том случае
